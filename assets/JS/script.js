...
for (let i = 0; i < cityButtons.length; i++) {
  cityButtons[i].addEventListener("click", function (event) {
    var cities = event.target.dataset.city;
    $.ajax({
      //const city = $("#city-input").val();
      url: `${BASE_URL}weather?q=${cities}&units=metric&appid=${API_KEY}`,
      method: "GET",
      success: function (response) {
        // Extract relevant data from response
        const cityName = response.name;
    ...
    $.ajax({
      url: `${BASE_URL}forecast?q=${cities}&units=metric&cnt=5&appid=${API_KEY}`,
      method: "GET",
      success: function (response) {
        // Extract relevant data from response
        const forecast = response.list.map((day) => ({
